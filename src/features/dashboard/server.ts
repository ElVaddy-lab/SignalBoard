...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  if (authError || !user) throw new Error("You must be signed in to view the Dashboard.");
  ...
  const [metricsResult, distributionResult, trendResult, upcomingResult, recentResult, activityResult] = await Promise.all([
    supabase.rpc("get_dashboard_metrics", { p_timezone: timezone, p_local_date: undefined }),
    supabase.rpc("get_status_distribution"),
    supabase.rpc("get_completion_trend", { p_timezone: timezone }),
    supabase.rpc("get_upcoming_deadlines", { p_local_date: dateInTimezone(timezone) }),
  ...
  if (metricsResult.error || distributionResult.error || trendResult.error || upcomingResult.error || recentResult.error || activityResult.error) throw new Error("We couldn\u2019t load your Dashboard.");
