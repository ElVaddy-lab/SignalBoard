...
import { projectPriorities, projectStatuses, type ProjectPriority, type ProjectStatus } from "@/data/projects";
...
export const createProjectSchema = z.object({
  ...
  deadline: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Use a valid calendar date").optional().or(z.literal("")).transform((value) => value || undefined),
});

export type ProjectInput = z.output<typeof createProjectSchema>;

export const projectSortOptions = ["updated-desc", "created-desc", "deadline-asc", "title-asc", "priority-desc"] as const;
...
export function parseProjectListParams(params: SearchParamRecord): ProjectListParams {
  const q = (first(getValue(params, "q")) ?? "").trim();
  const statuses = list(getValue(params, "status")).filter((value): value is ProjectStatus => projectStatuses.includes(value as ProjectStatus));
  const priorities = list(getValue(params, "priority")).filter((value): value is ProjectPriority => projectPriorities.includes(value as ProjectPriority));
  ...
  return {
    ...
    deadline: deadlineFilters.includes(deadlineCandidate as DeadlineFilter) ? deadlineCandidate as DeadlineFilter : "any",
    sort: projectSortOptions.includes(sortCandidate as ProjectSort) ? sortCandidate as ProjectSort : "updated-desc",
